...
def list_archive_items(category: str = "all", search: str | None = None, limit: int = 100) -> list[dict]:
    ...
    shaped to match the frontend's ArchiveItem interface:
    { id, title, category, author, description, date }.
    """
    results: list[dict] = []
    ...
    with psycopg2.connect(DB_DSN) as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            if category in ("all", "memorials"):
                query = """
                    SELECT id, name, description, place_type
...
def _item_type_to_category(item_type: str) -> str:
    if item_type == "speech":
        return "speeches"
    if item_type in ("manuscript", "letter", "constitutional_debate"):
        return "manuscripts"
    return "manuscripts"
